...
## Drop extra rows
The original data pull mistakenly included rows beyond the end of program year 1 for Pepsi and Wellstar. We drop them here. Note that we verified it did not impact the matching process.
``` {r filter-rows}
df_matched_orig <- df_matched_orig %>%
...
m1_rx_yes_match <- lmer(rx_pmpm_trunc ~ 1 + spring_dummy + spring_dummy*phase + customer_name +
             ...
             (1 | subclass_nn),
           weights = weights_nn,
           data = filter(df_matched, customer_name != "Hearst" & customer_name != "Tegna"))
...
# Program-year ROI multiple
deltamethod(m1_doubly_robust, "0 = (1 + (-1*b16 * sum(df_roi$member_months_treated)) / total_invoiced)")
