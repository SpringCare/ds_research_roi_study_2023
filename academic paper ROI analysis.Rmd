...
## Gross
m1_gross_no_match <- lmer(med_pmpm_trunc ~ 1 + spring_dummy + spring_dummy*phase + Hearst + DocuSign + Tegna + Wellstar +
             (1 | carrier_member_id),
           data = filter(df_analyze_all, index_date >= spring_start_date))

m1_gross_yes_match <- lmer(med_pmpm_trunc ~ 1 + spring_dummy + spring_dummy*phase + Hearst + DocuSign + Tegna + Wellstar +
             (1 | carrier_member_id) +
             (1 | subclass_nn),
           weights = weights_nn,
           data = df_matched)
...
# Sensitivity
tab_model(m1_gross_no_match, m1_gross_yes_match, m1_mh, m1_nonmh,
          show.ci=F, 
          show.se=T, 
...
df_plot_total <- predictions(m1_gross_yes_match, newdata = df_plot, re.form = ~0)
df_plot_mh <- predictions(m1_mh, newdata = df_plot, re.form = ~0) %>%
  mutate(across(c(estimate, conf.low, conf.high), ~
                  ifelse(spring_dummy == "Spring" & phase == "post_tx", .x + spring_pmpm_adjustment, .x)))
...
## Model
m1 <- lmer(med_pmpm_trunc ~ 1 + spring_dummy + spring_dummy*phase + 
...
m2 <- lmer(med_pmpm_trunc ~ 1 + spring_dummy + spring_dummy*phase + 
             cc_any + cc_any*(spring_dummy + spring_dummy*phase) +
             customer_name +
             (1 | carrier_member_id) +
             (1 | subclass_nn),
...
m3 <- lmer(med_pmpm_trunc ~ 1 + spring_dummy + spring_dummy*phase + 
             cc_any + cc_any*(spring_dummy + spring_dummy*phase) +
             customer_name +
             (1 | carrier_member_id) +
             (1 | subclass_nn),
...
m4 <- lmer(med_pmpm_trunc ~ 1 + spring_dummy + spring_dummy*phase + 
             cc_any + cc_any*(spring_dummy + spring_dummy*phase) +
             customer_name +
             (1 | carrier_member_id) +
             (1 | subclass_nn),
...
m5 <- lmer(med_pmpm_trunc ~ 1 + spring_dummy + spring_dummy*phase + 
             cc_any + cc_any*(spring_dummy + spring_dummy*phase) +
             customer_name +
             (1 | carrier_member_id) +
             (1 | subclass_nn),
...
m_no_cc <- lmer(med_pmpm_trunc ~ 1 + spring_dummy + spring_dummy*phase + 
             cc_any + cc_any*(spring_dummy + spring_dummy*phase) +
             customer_name +
             (1 | carrier_member_id) +
             (1 | subclass_nn),
...
tab_model(m1, m2, m3, m4, m5, m_no_cc,
...
for (i in 1:length(m_list)) {
  ...
  df_cc_results[length(m_list) + 1,] <- hypotheses(m_no_cc, hypothesis = "b9 = 0")
}
...
df_cc_results <- df_cc_results %>%
  mutate(
    ...
    var_name = c("cc_diabetes", "cc_hypertension", "cc_asthma_copd", "excl_pregnancy", "cc_cancer", "No chronic condition")
  )
...
ggplot(df_plot_cc, aes(x=Estimate, xmin=lb, xmax=ub, y=chronic_condition)) +
  ...
  geom_vline(xintercept = 344, linetype = "dashed", color = "grey") +
  annotate(geom = "text", x=344, y=6.5, label = "Population average = 344", hjust=-0.07, color = "grey") +
  expand_limits(y = c(1, 6.7)) +
  theme_modern() +
  theme(legend.position = "bottom") +
  labs(
    x = "PMPM savings",
...
df_cost_results <- data.frame(
  outcome = rep(c("Office visits", "Emergency room", "Hospital outpatient", "Hospital inpatient", "Therapy"), 2),
  spend_type = c(rep("Mental healthcare savings", 5), rep("All other healthcare savings", 5)),
  estimate = NA,
  se = NA
) %>%
...
df_cost_results <- df_cost_results %>%
    mutate(
      estimate = case_when(
        outcome == "Therapy" & spend_type == "Mental healthcare savings" ~ estimate + spring_pmpm_adjustment,
        TRUE ~ estimate
      )) %>%
  mutate(
...
donut_list[[i]] <- ggplot(filter(df_donut_cc, outcome == outcome_list[[i]]), aes(ymax=ymax, ymin=ymin, xmax=4, xmin=3, fill=spend_type)) +
  ...
  coord_polar(theta="y") + 
  xlim(c(2, 4)) +
  annotate(geom = "text", label = paste0(round(100*df_donut_cc$fraction[i],0), "%"), x=2, y=0, size = 20) +
  scale_fill_manual(values = cc_colors) + 
  theme_modern() +
  theme_void() +
...
# Dot plot for paper
# Make dotplot
ggplot(df_cost_results, aes(x=estimate, xmin=conf.low, xmax=conf.high, y=outcome, group_by = spend_type, color = spend_type)) +
  geom_pointrange(position = position_dodge(width=.5)) +
  #geom_text(aes(x=estimate, y=outcome, label = round(estimate, 0), fill = NULL, vjust = -1), colour = "#8e8e8e", size = 3) +
  #expand_limits(y = c(1, 6.7)) +
  geom_vline(xintercept = 0, linetype = "dashed") +
  theme_modern() +
  theme(legend.position = "bottom") +
...
# Those standard errors look bad, probably because there's not much MH spend on the Spring side.
# Let's collapse to service categories.
df_matched <- df_matched %>%
  mutate(
    med_pmpm_trunc_ov = med_pmpm_trunc_ov_mh + med_pmpm_trunc_ov_nonmh,
    med_pmpm_trunc_er = med_pmpm_trunc_er_mh + med_pmpm_trunc_er_nonmh,
    med_pmpm_trunc_hosp_op = med_pmpm_trunc_hosp_op_mh + med_pmpm_trunc_hosp_op_nonmh,
    med_pmpm_trunc_ip = med_pmpm_trunc_ip_mh + med_pmpm_trunc_ip_nonmh,
    med_pmpm_trunc_therapy = med_pmpm_trunc_therapy_mh + med_pmpm_trunc_therapy_nonmh,
    med_pmpm_trunc_other = med_pmpm_trunc_other_mh + med_pmpm_trunc_other_nonmh
  )

cc11 <- lmer(med_pmpm_trunc_ov ~ 1 + spring_dummy + spring_dummy*phase +
              customer_name +
              (1 | carrier_member_id) +
              (1 | subclass_nn),
            weights = weights_nn,
...
cc12 <- lmer(med_pmpm_trunc_er ~ 1 + spring_dummy + spring_dummy*phase +
              customer_name +
              (1 | carrier_member_id) +
              (1 | subclass_nn),
            weights = weights_nn,
...
cc13 <- lmer(med_pmpm_trunc_hosp_op ~ 1 + spring_dummy + spring_dummy*phase +
              customer_name +
              (1 | carrier_member_id) +
              (1 | subclass_nn),
            weights = weights_nn,
...
cc14 <- lmer(med_pmpm_trunc_ip ~ 1 + spring_dummy + spring_dummy*phase +
              customer_name +
              (1 | carrier_member_id) +
              (1 | subclass_nn),
            weights = weights_nn,
...
cc15 <- lmer(med_pmpm_trunc_therapy ~ 1 + spring_dummy + spring_dummy*phase +
              customer_name +
              (1 | carrier_member_id) +
              (1 | subclass_nn),
            weights = weights_nn,
...
cc16 <- lmer(med_pmpm_trunc_other ~ 1 + spring_dummy + spring_dummy*phase +
              customer_name +
              (1 | carrier_member_id) +
              (1 | subclass_nn),
            weights = weights_nn,
...
# Make in table
df_cost_results_collapsed <- data.frame(
  outcome = c("Office visits", "Emergency room", "Hospital outpatient", "Hospital inpatient", "Therapy", "Other"),
  estimate = NA,
  se = NA
) %>%
  mutate(outcome = fct_relevel(as.factor(outcome),"Other", "Therapy", "Emergency room", "Office visits", "Hospital outpatient", "Hospital inpatient"))

for (i in 11:16) {
  df_cost_results_collapsed$estimate[i-10] = summary(get(paste0("cc", i)))$coefficients["spring_dummySpring:phasepost_tx", "Estimate"]
  df_cost_results_collapsed$se[i-10] = summary(get(paste0("cc", i)))$coefficients["spring_dummySpring:phasepost_tx", "Std. Error"]
}

# Adjust therapy costs for Spring fees
df_plot_cost_results_collapsed <- df_cost_results_collapsed %>%
    mutate(
      estimate = case_when(
        outcome == "Therapy" ~ estimate + spring_pmpm_adjustment,
        TRUE ~ estimate
      )) %>%
  mutate(
    conf.low = estimate - 1.96*se,
...
# Make collapsed dotplot
ggplot(df_plot_cost_results_collapsed, aes(x=estimate, xmin=conf.low, xmax=conf.high, y=outcome)) +
  geom_pointrange(position = position_dodge(width=.5)) +
  geom_text(aes(x=estimate, y=outcome, label = round(estimate, 0), fill = NULL, vjust = -1), colour = "#8e8e8e", size = 3) +
...
ggplot(df_matched, aes(x=therapy_sessions, y = med_pmpm_trunc_nonmh)) +
  geom_point() +
  geom_smooth() +
...
df_matched <- df_matched %>%
  mutate(
    therapy_sessions_factor = case_when(
      therapy_sessions == 0 ~ "0",
      therapy_sessions > 0 & therapy_sessions <= 2 ~ "1 to 2",
      therapy_sessions > 2 & therapy_sessions <= 5 ~ "3 to 5",
      therapy_sessions > 5 & therapy_sessions <= 12 ~ "6 to 12",
      therapy_sessions > 12 ~ "13+"
    ),
    any_therapy = ifelse(therapy_sessions > 0, "yes", "no")
  ) %>%
...
m_sessions1 <- lmer(med_pmpm_trunc_nonmh ~ 1 + spring_dummy + spring_dummy*phase + Hearst + Pepsi + Tegna + Wellstar +
                     as.factor(months_post) + as.factor(months_post)*spring_dummy + 
                      any_therapy + therapy_sessions + therapy_sessions*spring_dummy + 
             (1 | carrier_member_id) +
             (1 | subclass_nn),
           weights = weights_nn,
           data = df_matched)
...
deltamethod(m1_gross_yes_match, "0 = (-1*by*(sum(df_roi$months_treated*df_roi$n)))/total_invoiced")
...
deltamethod(m1_gross_yes_match, "0 = (-1*b8*12*3640)/total_invoiced")
```
...
# Hypothesis test including spring costs
hypotheses(m1_gross_yes_match, "b8 + spring_pmpm_adjustment = 0")
hypotheses(m1_mh, "b8 + spring_pmpm_adjustment = 0")
hypotheses(m1_nonmh, "b8 = 0")

# First Program-year PMPY
hypotheses(m1_gross_yes_match, "0 = -1 * (b8 + spring_pmpm_adjustment) * mean_months_treated")

# First program year net savings %
hypotheses(m1_gross_yes_match, "0 = (6.5*(b8 + spring_pmpm_adjustment)) / ((12-6.5)*df_plot_total$estimate[1] + 6.5*(df_plot_total$estimate[3]))")

# Plot
df_plot_net <- df_plot_total %>%
  mutate(
    estimate = case_when(
      spring_dummy == "Spring" & phase == "post_tx" ~ estimate + spring_pmpm_adjustment,
      TRUE ~ estimate)
    ) %>%
  mutate(
...
# Overall with error bars
ggplot(df_plot_net, aes(x=phase_label, y=estimate, ymin=conf.low, ymax=conf.high, group = spring_dummy, color = spring_dummy)) +
  geom_errorbar(width = 0.05) +
  geom_point(size=3) +
  geom_line() +
  scale_color_manual(values = color_scheme, labels = c("Control", "Program")) +
  geom_text_repel(aes(label=round(estimate, 0)), hjust = ifelse(df_plot$phase_label == "Before MH diagnosis", 1.4, -0.4)) +
  annotate(geom = "text", label = "Difference-in-differences = $151 PMPM", x = 1.9, y=500, color = "#37837c") +
  theme_modern() +
  theme(legend.position = "bottom") +
  labs(
    title = "Total Medical Costs",
    x = "",
    y = "PMPM allowed amount",
    color = ""
  )
...
# Plot total mental health spending
ggplot(df_plot_mh, aes(x=phase_label, y=estimate, ymin=conf.low, ymax=conf.high, group = spring_dummy, color = spring_dummy)) +
  geom_errorbar(width = 0.05) +
  geom_point(size=3) +
  geom_line() +
  scale_color_manual(values = color_scheme, labels = c("Control", "Program")) +
  ...
  theme(legend.position = "none") +
  labs(
    title = "Mental Health Costs",
    x = "",
    y = "PMPM allowed amount",
    color = ""
  )

# Plot physical health spending
ggplot(df_plot_nonmh, aes(x=phase_label, y=estimate, ymin=conf.low, ymax=conf.high, group = spring_dummy, color = spring_dummy)) +
  geom_errorbar(width = 0.05) +
  geom_point(size=3) +
  geom_line() +
  scale_color_manual(values = color_scheme, labels = c("Control", "Program")) +
...
## Make barplots
df_bar_plot <- rbind(
  hypotheses(m1_mh, "b3 = 0"),
  hypotheses(m1_mh, "b3 + b8 + spring_pmpm_adjustment = 0"),
  hypotheses(m1_nonmh, "b3 = 0"),
  hypotheses(m1_nonmh, "b3 + b8 = 0")
) %>%
  data.frame() %>%
  mutate(spring_dummy = c("Control", "Program", "Control", "Program")) %>%
  mutate(spend_type = c("Mental health spend", "Mental health spend", "Physical health spend", "Physical health spend"))

ggplot(df_bar_plot, aes(x=spend_type, y=estimate, ymin=conf.low, ymax=conf.high, group = spring_dummy, fill = spring_dummy)) +
  geom_bar(stat = "Identity", position = position_dodge()) +
  geom_errorbar(width = 0.05, position = position_dodge(1)) +
  geom_text(aes(label=round(estimate, 0)), position = position_dodge(1), vjust = -1, hjust=-0.6, colour = "black") +
  scale_fill_manual(values = color_scheme) +
  theme_modern() +
  theme(legend.position = "bottom") +
  labs(
    title = "Changes in medical spending after a mental health diagnosis",
    x = "",
    y = "PMPM allowed amount",
    fill = ""
  )
...
## Net chronic conditions
``` {r net-chronic-conditions}
## Make it nice
df_plot_cc_net <- df_cc_results %>%
  mutate(
    Estimate = -1*Estimate - spring_pmpm_adjustment,
    lb = -1*lb - spring_pmpm_adjustment,
    ub = -1*ub - spring_pmpm_adjustment
  ) %>%
  mutate(chronic_condition = fct_reorder(as.factor(chronic_condition), Estimate))

# Make dotplot
ggplot(df_plot_cc_net, aes(x=Estimate, xmin=lb, xmax=ub, y=chronic_condition)) +
  geom_pointrange(position = position_dodge(width=0.5)) +
  geom_text(aes(x=Estimate, y=chronic_condition, label = round(Estimate, 0), fill = NULL, vjust = -1), colour = "#8e8e8e", size = 3) +
  geom_vline(xintercept = 344 - spring_pmpm_adjustment, linetype = "dashed", color = "grey") +
  annotate(geom = "text", x=344 - spring_pmpm_adjustment, y=6.5, label = paste0("Population average = ", round(344 - spring_pmpm_adjustment, 0)), hjust=-0.07, color = "grey") +
  expand_limits(y = c(1, 6.7)) +
  theme_modern() +
  theme(legend.position = "bottom") +
  labs(
...
```
## Do session counts differ for individuals with chronic conditoins?
``` {r cc-session-counts}
...
We have more gross savings on mental health spend compared to the control group members that did engage. Non-MH savings are the same in both groups. Don't love that finding as it undermines the narrative the MH treatment provides physical health savings. 
...
m1_bh_overall <- lmer(med_pmpm_trunc ~ 1 + spring_dummy_stratified + spring_dummy_stratified:phase +
             (1 | carrier_member_id) +
             (1 | subclass_nn),
           weights = weights_nn,
           data = df_matched)
  
m1_bh_mh_spend <- lmer(med_pmpm_trunc_mh ~ 1 + spring_dummy_stratified + spring_dummy_stratified:phase +
             (1 | carrier_member_id) +
             (1 | subclass_nn),
           weights = weights_nn,
           data = df_matched)
...
m1_drop_month1_ip <- lmer(med_pmpm_trunc ~ 1 + spring_dummy + spring_dummy*phase + Hearst + DocuSign + Tegna + Wellstar +
             (1 | carrier_member_id) +
             (1 | subclass_nn),
           weights = weights_nn,
           data = filter(df_matched, ip_month1==0))

tab_model(m1_gross_yes_match, m1_drop_month1_ip, show.ci=F, show.se=T,
