...
df_matched_orig <- readRDS(paste0(Sys.getenv("claims_folder_path"), "data/roi_study_2023/df_matched.Rdata"))
...
df_matched <- df_matched_orig %>%
  ...
  mutate(across(c(med_pmpm, 
                ...
                ~ifelse(customer_name == "Hearst", .x/.8, .x)
  )) %>%
  #Add net
  mutate(
...
## Net spending
m1_gross <- lmer(med_pmpm_trunc ~ 1 + spring_dummy + spring_dummy*phase + Hearst + DocuSign + Tegna + Wellstar +
             (1 | carrier_member_id) +
             (1 | subclass_nn),
           weights = weights_nn,
           data = df_matched)
...
deltamethod(m1_gross, "0 = (6.5*b8) / ((12-6.5)*df_plot_total$estimate[1] + 6.5*(df_plot_total$estimate[2]))") # (PMPM savings / total control group spend)

# First Program-year PMPY
deltamethod(m1_gross, "0 = -1 * b8 * mean_months_treated")

# Program-year ROI multiple
deltamethod(m1_net, "0 = (1 + (-1*b8 * sum(df_roi$member_months_treated)) / total_invoiced)")
