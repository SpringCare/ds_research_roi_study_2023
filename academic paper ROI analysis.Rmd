...
# Sensitivity: any baseline pharm spend
df_matched <- df_matched %>%
  group_by(carrier_member_id) %>%
  mutate(rx_baseline = mean(rx_pmpm_trunc[phase == "pre_tx"])) %>%
  mutate(rx_any_spend = ifelse(rx_baseline > 0, "yes", "no"))

m1_rx_base_yes <- lmer(rx_pmpm_trunc ~ 1 + spring_dummy + spring_dummy*phase + customer_name +
             (1 | carrier_member_id) +
             (1 | subclass_nn),
           weights = weights_nn,
           data = filter(df_matched, rx_any_spend == "yes"))

tab_model(m1_rx_yes_match, m1_rx_base_yes, m1_rx_truncate10k, m1_rx_truncate5k, m1_rx_truncate2.5k, show.ci=F, show.se=T, 
          pred.labels = c("Baseline PMPM", "Baseline difference (Spring - Control)", "Control pre-post change", "Hearst", "DocuSign", "Tegna", "Wellstar", "Diff-in-diff (Spring change - Control change)"),
...
df_cost_results <- df_cost_results %>%
    mutate(
      estimate = case_when(
        outcome == "Therapy" & spend_type == "Mental healthcare savings" ~ estimate + spring_cost_adjustment,
        TRUE ~ estimate
      )) %>%
  mutate(
...
df_plot_cost_results_collapsed <- df_cost_results_collapsed %>%
    mutate(
      estimate = case_when(
        outcome == "Therapy" ~ estimate + spring_cost_adjustment,
        TRUE ~ estimate
      )) %>%
  mutate(
    conf.low = estimate - 1.96*se,
...
hypotheses(m1_gross_yes_match, "b8 + spring_cost_adjustment = 0")
hypotheses(m1_mh, "b8 + spring_cost_adjustment = 0")
hypotheses(m1_nonmh, "b8 = 0")

# First Program-year PMPY
hypotheses(m1_gross_yes_match, "0 = -1 * (b8 + spring_cost_adjustment) * mean_months_treated")

# First program year net savings %
hypotheses(m1_gross_yes_match, "0 = (6.5*(b8 + spring_cost_adjustment)) / ((12-6.5)*df_plot_total$estimate[1] + 6.5*(df_plot_total$estimate[3]))")

# Plot
df_plot_net <- df_plot_total %>%
  mutate(
    estimate = case_when(
      spring_dummy == "Spring" & phase == "post_tx" ~ estimate + spring_cost_adjustment,
      TRUE ~ estimate)
    ) %>%
  mutate(
...
df_bar_plot <- rbind(
  ...
  hypotheses(m1_mh, "b3 + b8 + spring_cost_adjustment = 0"),
